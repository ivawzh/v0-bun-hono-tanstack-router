...
server.tool(
  ...
  "Update task fields during workflow stages.",
  {
    ...
    updates: z.object({
      ...
      plan: z.union([z.string(), z.object({}).passthrough(), z.array(z.any())]).optional(),
  ...
  async ({ taskId, updates }, { requestInfo }) => {
    ...
    try {
      ...
      logger.tool("task.update", "success", { taskId, updatedFields: Object.keys(updates as Record<string, any>) });
